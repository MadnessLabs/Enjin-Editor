...
(async function () {
    ...
    if (load !== null) {
      ...
      if (load) {
        files.push(
          ...load
            .split(/,/)
            .map((c) => (/\./.test(c) ? c : `prism-live-${c}.js`))
        );
      }
  ...
  var _ = (Prism.Live = class PrismLive {
    constructor(source) {
          ...
          if (evt.key == "Tab" && !evt.altKey) {
            ...
            if (this.tabstops && this.tabstops.length > 0) {
              ...
              if (outdent) {
                ...
                this.selectionStart +=
                  before.length + 1 - (outdent + isBeforeIndented);
              } else {
                // Indent
      ...
      requestAnimationFrame(() => {
        ...
        this.pre.style.height =
          this.source.style.height || sourceCS.getPropertyValue("--height");
        this.pre.style.maxHeight =
          this.source.style.maxHeight ||
          sourceCS.getPropertyValue("--max-height");
      });
      ...
      this.source.dispatchEvent(
        new CustomEvent("prism-live-init", { bubbles: true, detail: this })
      );
    }
    ...
    toggleComment() {
      ...
      if (commentNode) {
        ...
        if (
          comments.singleline &&
          commentText.indexOf(comments.singleline) === 0
        ) {
          var end = start + commentText.length;
          this.set(this.value.slice(start + comments.singleline.length, end), {
            start,
            end,
          });
    ...
    delete(characters, { forward, pos } = {}) {
      var i = (characters =
        characters > 0 ? characters : (characters + "").length);
    ...
    static adjustIndentation(
      text,
      { indentation, relative = true, indent = _.DEFAULT_INDENT }
    ) {
      if (!relative) {
  ...
  Object.assign(_, {
    ...
    regexp: (() => {
      ...
      var _regexp = (flags, strings, ...values) => {
        var pattern =
          strings[0] +
          values.map((v, i) => escape(v) + strings[i + 1]).join("");
  ...
  $.ready().then(() => {
    $$(
      ":not(.prism-live) > textarea.prism-live, :not(.prism-live) > pre.prism-live"
    ).forEach((source) => {
      if (!_.all.get(source)) {
        new _(source);
      }
